...
// animated: no          <- time не используется
// samples: yes          <- читает tex не в uv, значит нужен более грубый damage
...
- **заголовок**, каждая строка — комментарий `//` вида `ключ: значение`. У любого ключа есть значение по умолчанию, так что эффект без заголовка заработает — но плохо: в конце меню под именем файла и с 🎬, и отрисованный так, будто он не двигается и не сэмплит. Два последних ключа как раз те, которые стоит проставить правильно — см. [режим отрисовки](#режим-отрисовки-объявляется-тобой)
...
```glsl
// animated: yes     <- в теле используется time
// samples: yes      <- в теле читается tex не в точке uv
```
