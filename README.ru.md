...
- **три строки заголовка**, каждая — комментарий `//` вида `ключ: значение`. Без них эффект всё равно заработает, но окажется в конце меню под именем файла и с 🎬
...
### Единственное, что не объявляется, — режим отрисовки

Hyprland надо сказать, насколько усердно перерисовывать, и **это читается из твоего кода**, а не из заголовка:
...
Перед этой проверкой комментарии вырезаются, так что упоминание `time` в прозе не сделает статический эффект анимированным. Вызов `texture()` заодно ставит эффект в цепочке **первым**, перед цветовыми фильтрами: сначала геометрия, потом покраска результата.
