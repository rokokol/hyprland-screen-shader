...
| `~/.local/state/huix/shader` | durable-состояние (stack, bright, slot) — переживает логаут/ребут |
...
| Клавиши | Действие |
|---------|----------|
| `SUPER+G` | ч/б в стопку / из стопки (`effect toggle grayscale`) |
| `SUPER+SHIFT+G` | rofi-пикер всех эффектов |
| `SUPER+CTRL+]` / `[` | яркость +/− 0.05 (зажатие повторяет) |
...
screen-shader.sh bright up|down        # яркость ±0.05 (кламп 0.10..2.00)
...
screen-shader.sh menu                  # строки "<эмодзи> <подпись>|<имя>"; активные
                                       # помечены номером применения (01. 02. 03.)
```
...
1. Создать `scripts/shaders/<name>.frag` с **только** функцией
   ...
   Имена хелперов могут совпадать между эффектами: при композиции (стопка,
   `flash` поверх неё) менеджер сам переименует определения последующих тел
   (суффиксы `_1`, `_2`, …).
...
4. `git add` новый файл (sync берёт только отслеживаемые).
