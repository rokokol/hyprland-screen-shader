...
| `SUPER+CTRL+]` / `[` | яркость +/− 0.10 (зажатие повторяет) |
| `SUPER+CTRL+Backspace` | сброс яркости в 1.00 |

Индикатор в waybar (`custom/shader`, оба хоста): ЛКМ — пикер, ПКМ — снять
...
- **Индикатор waybar обновляется сигналом** `SIGRTMIN+N`, где `N` задаётся
  **один раз** в `waybar/shader.nix` (`shaderSignal`) и пробрасывается скрипту через
